                        ...
                        if(videos != null && audios != null && books != null) {
                            if(Array.isArray(videos) && Array.isArray(audios) && Array.isArray(books)) {
                                if(videos.length > 0 || audios.length > 0 || books.length > 0) {
                                    if(! allIsUpload() || ! allFilesIsUpload()) {
...
function StartUploadingVideos(playlist,lang,secsses) {
    ...
    if(videos[currentUpload].id == -1) {
        var tempForm = videos[currentUpload].getFormData();
    } else {
        var tempForm = videos[currentUpload].getUpdateFormData();
    }
...
function updateVideos() {
    ...
    for(var i = 0; i < videos.length; i++) {
        if(videos[i] != undefined && videos[i] != null) {
            if(videos[i].uploaded && videos[i].updated) {
                var updatedFormData = videos[i].getUpdateFormData();
                if(updatedFormData != null) {
                    updatedFormData.append('_token',lang[9]);
                    ajaxRequest('post',window.location.origin + '/admin/playlist/update/video',updatedFormData);
                }
            } else if(videos[i].updated) {
                videos[i].createWatingVideoElement();
            }
        }
    }
}