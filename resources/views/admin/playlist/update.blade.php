...
@section('title') {{ __('headers.admin-navbar-update-playlist') }} @endsection
...
@section('content')
    <header class="small-default-header"><span>{{ __('headers.admin-navbar-update-playlist') }}</span></header>
    <form id="mainForm" class="add-playlist-form" method="post" action="" enctype="multipart/form-data">
        ...
        <section>
            <div class="select-profile-image no-select">
				<div class="transition">
					<img id="imageUploading" src="{{ ($playlist->poster != null) ? asset($playlist->poster) : asset('/images/static/playlist-default.png') }}"/>
        ...
        <section>
            <div><span class="no-select">{{ __('masseges.playlist-title') }}</span><input id="playlistTitleInput" type="text" name="title" autocomplete="off" placeholder="{{ __('masseges.playlist-title') }}" value="{{ $playlist->title }}"/></div>
            <div><span class="no-select">{{ __('masseges.playlist-price') }}</span><input id="playlistPriceInput" type="text" name="price" autocomplete="off" placeholder="{{ __('masseges.playlist-price') }}" value="{{ $playlist->price }}"/></div>
            <div><span class="no-select">{{ __('masseges.playlist-description') }}</span><textarea id="playlistDescInput" name="description" autocomplete="off" placeholder="{{ __('masseges.playlist-description') }}">{{ ($playlist->description != null) ? $playlist->description :  '' }}</textarea></div>
            <div><span class="no-select">{{ __('masseges.date-of-available-playlist') }}</span><input id="playlistPriceInput" type="date" name="availability_time" autocomplete="off" value="{{ ($playlist->description != null) ? Date('Y-m-d',strtotime($playlist->availability_time)) : '' }}" /></div>
        </section>
        ...
        <div class="select-type">
            ...
            <div id="typesOfThisPlaylist" class="types-of-this-playlist no-select">
                ...
                @foreach($typesOfThisPlaylist as $typeOfThisPlaylist)
                    <div class="type" id="typeOfPlaylist{{ $typeOfThisPlaylist->id }}">
                        <span>{{ $typeOfThisPlaylist->name }}<input type="hidden" value="{{ $typeOfThisPlaylist->id }}" name="types[]" /></span>
                        <canvas width="15" height="15" onclick="removeThisType({{ $typeOfThisPlaylist->id }});"></canvas>
                    </div>
                @endforeach
        ...
        <div id="addNewVideoTemplate" class="pop-up-template template-of-this-playlist" style="display: none;">
            ...
            <div class="add-new-type add-new-video">
                <header class="small-default-header add-video-header"><span>{{ __('masseges.add-video') }}</span></header>
    ...
    <div class="footer-of-add-playlist-form"><input type="button" value="{{ __('masseges.save') }}" onclick="updatePlaylist();" /></div>
...
@section('scripts')
    ...
    <script type="text/javascript" lang="javascript" src="{{ asset('js/admin/playlist/update.js') }}"></script>
    <script type="text/javascript" lang="javascript">
        ...
        if(typesOfThisPlaylist != null) {
            var tempDeleteButtonsOfType = typesOfThisPlaylist.getElementsByTagName('canvas');
            if(typeof(tempDeleteButtonsOfType) == "object") {
                if(tempDeleteButtonsOfType.constructor == HTMLCollection) {
                    for(var i = 0; i < tempDeleteButtonsOfType.length; i++) {
                        if(typeof(tempDeleteButtonsOfType[i]) == "object") {
                            tempDeleteButtonsOfType[i].width = 15;
                            tempDeleteButtonsOfType[i].height = 15;
                            drawRemoveIconCanvas(tempDeleteButtonsOfType[i],'#ffffff');
                        }
                    }
                }
            }
        ...
        var langOfVideoElement = [
                '{{ __('masseges.size') }}',
                '{{ asset('/images/static/video-default.jpg') }}',
                '{{ __('masseges.update') }}',
                '{{ __('masseges.general-error') }}',
                '{{ __('masseges.ask-remove-video') }}',
                '{{ __('masseges.delete') }}',
                '{{ __('input.character-of-title-must-min-than-255') }}',
                '{{ __('masseges.show') }}',
        ];
        var langOfWatingVideoElement = [
            '{{ asset('/images/static/video-default.jpg') }}',
        ...
        if(Array.isArray(videos)) {
            ...
            videos.push(new Video(null,null,null,null,null,null,@json($video)));
            @endforeach
        }
        ...
        if(typeof(Audio) == "function") {
            @foreach($audios as $audio)
                var tempAudioObject = new Audio(null,null,null,null,null,null,null,@json($audio));
                tempAudioObject.blobType = "audio";
            @endforeach
        }
        if(typeof(Book) == "function") {
            @foreach($books as $book)
                var tempBookObject = new Book(null,null,null,null,null,null,null,@json($book));
                tempBookObject.blobType = "book";
            @endforeach
        }
        /*############################*/
        ...
        function getPlaylistLang() {
            return [
                ...
                '{{ route('admin.playlist.save') }}',
                '{{ __('masseges.update-playlist-ok-wait-upload-new-videos') }}',
