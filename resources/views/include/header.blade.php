<header id="header" class="header-after-scroll-down no-select"><nav>
            <ul>
                ...
                @else
                    ...
                    <a class="notifcations-button" id="notifcationButton" ><li class="icon"><!--<span class="fa">&#xf0f3;</span>!--><span class="material-icons">notifications</span>@if(session()->has('newNotifcations')) @if(session()->get('newNotifcations') > 0)<span class="count-of-new-notifcations">{{ session()->get('newNotifcations') }}</span>@endif @endif</li></a>
                @endguest
            </ul>
